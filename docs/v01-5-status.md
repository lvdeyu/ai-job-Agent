...
- 暂未实现真实 `pgvector Top-K` embedding 检索；已保留 `retrieval_mode` 边界。
- 暂未接入 `LangGraph` 和 checkpoint；当前由后端服务函数控制选题、评分、追问、终止和报告生成。
- 当前题库仍然只有项目内置 3 道种子题，适合验证链路，不代表最终面试质量。
- 当前暂停/恢复能力通过持久化会话和轮次支持基础查看，尚未实现 LangGraph 级 checkpoint 恢复。
